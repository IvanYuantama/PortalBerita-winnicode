...
const RegisterJurnalis = () => {
  ...
  const checkUsernameExists = async (username) => {
    try {
      const response = await axios.get("https://portal-berita-winnicode.vercel.app/user/getAllJurnalis");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (uploadResponse.status === 200) {
        ...
        const response = await axios.post("https://portal-berita-winnicode.vercel.app/user/signupJurnalis", updatedFormData);
  ...
  return (
    <div className="flex justify-center items-center min-h-screen" style={{ backgroundColor: "black" }}>
      <form className="flex flex-col bg-white p-8 rounded shadow-md w-full max-w-md" onSubmit={handleSubmit}>
        <h2 className="text-2xl font-bold mb-4 text-center text-black">Register Jurnalis</h2>
        <div className="mb-4">
...
export default RegisterJurnalis;
